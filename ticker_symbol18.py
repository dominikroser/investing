...
def investing_method(selected_tickers, use_smoothed, selected_date, investing_method):
    ...
    for ticker_symbol in selected_tickers:
        ...
        try:
            st.write(f"Closing price on selected date for {ticker_symbol}: {df['Close'].loc[selected_date]}")
        except KeyError:
    ...
    if use_smoothed:
        ...
        for ticker_symbol in selected_tickers:
            plt.plot(joined_df['Date'], joined_df[f'Smoothed Percentage Change_{ticker_symbol}'], label=ticker_symbol)
    else:
        plt.title("Percentage Change")
        for ticker_symbol in selected_tickers:
            plt.plot(joined_df['Date'], joined_df[f'Percentage Change_{ticker_symbol}'], label=ticker_symbol)
