...
def calculate_percentage_change(selected_tickers, use_smoothed):
    ...
    for ticker_symbol in ticker_symbols:
        ...
        dfs.append(df[['Dates', 'Percentage Change' + ticker_symbol, 'Smoothed Percentage Change' + ticker_symbol]])
    ...
    if use_smoothed:
        ...
        selected_cols = ['Dates'] + [col for col in joined_df.columns if col.startswith('Smoothed Percentage Change')]
    else:
    ...
    # Plot the selected columns on a single graph
    st.line_chart(joined_df[selected_cols], use_container_width=True)
