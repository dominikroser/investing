...
def calculate_percentage_change(ticker_symbol1, ticker_symbol2, use_smoothed):
    ...
    if use_smoothed:
        st.line_chart1(data=df1, *, x=df1['Dates'], y=df1['Smoothed Percentage Change'], color=blue, width=0, height=0, use_container_width=True)
        st.line_chart2(data=df2, *, x=df2['Dates'], y=df2['Smoothed Percentage Change'], color=blue, width=0, height=0, use_container_width=True)
    else:
