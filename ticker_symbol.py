...
def relative_strength(ticker_symbol, comparison_symbol):
    ...
    closing_price_8months = df.loc[df['Date'] == specific_date, 'Close'].values
    
    df_comp = yf.download(comparison_symbol, start="2023-01-01", end=date.today())
    closing_today_comp = df_comp['Close'].iloc[-1]
    specific_date_comp = last_day_8_months_ago.strftime('%Y-%m-%d')
    closing_price_8months_comp = df_comp.loc[df_comp['Date'] == specific_date_comp, 'Close'].values
    ...
    plt.plot(df_comp.index, df_comp['Close'], label=comparison_symbol)
